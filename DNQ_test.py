...
"""
s 将会包括该轮中订单数、空闲司机数、司机之间的收入方差、平均数、jain、当前轮次T
"""
...
class DQN(object):
    ...
    def choose_action_eval(self, x):  # 定义动作选择函数 (x为状态)
        x = torch.unsqueeze(torch.FloatTensor(x), 0)  # 将x转换成32-bit floating point形式，并在dim=0增加维数为1的维度
        if np.random.uniform() < EPSILON:  # 生成一个在[0, 1)内的随机数，如果小于EPSILON，选择最优动作
            actions_value = self.net.forward(x)  # 通过对评估网络输入状态x，前向传播获得动作值
            action = torch.max(actions_value, 1)[1].data.numpy()  # 输出每一行最大值的索引，并转化为numpy ndarray形式
            action = action[0]  # 输出action的第一个数
        else:  # 随机选择动作
            action = np.random.randint(0, N_ACTIONS)  # 这里action随机等于0或1 (N_ACTIONS = 2)
...
# torch.save(dqn.eval_net, "eval_net_result.pkl")
#
#
# total_reward = 0
# obs = env.reset()
#
# while True:
#     action = dqn.choose_action_eval(obs)
