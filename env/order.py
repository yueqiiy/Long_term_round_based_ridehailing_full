...
from setting import FLOAT_ZERO, WAIT_TIMES, INT_ZERO

# __all__ = ["Order", "generate_road_orders_data", "generate_grid_orders_data"]
__all__ = ["Order", "generate_road_orders_data"]
...
class Order:
    ...
    def __init__(self, order_id: int, pick_location: PickLocation, drop_location: DropLocation, request_time: int, wait_time: int, order_distance: float, order_fare: float, n_riders: int):
    ...
    def load_orders_data(cls, start_time: int, time_slot: int, input_file: str):
        ...
        for csv_iterator in pd.read_table(input_file, chunksize=chunk_size, iterator=True):  # 这么弄主要是为了防止order_data过大
            for line in csv_iterator.values:
                # ["request_time", "wait_time", "pick_index", "drop_index", "order_distance", "order_fare", "n_riders"]
                ...
                n_riders = int(each_order_data[6])
                order = cls(
                    order_id=order_id,
                ...
                if request_time < current_time + time_slot:
                    ...
                    current_time += time_slot  #  这个顺序会导致第一个时间槽的数据输出为第二个时间槽
                    yield current_time, each_time_slot_orders
                    # yield current_time, each_time_slot_orders     # 这样写相当于刚开始就进行了匹配，而不是等到时间槽末尾才开始匹配
    ...
    def __repr__(self):
        return "(order_id: {0}, pick_up: {1}, drop_off: {2})".format(self._order_id, self.pick_location.osm_index, self.drop_location.osm_index)
    ...
    @real_pick_up_time.setter
    def real_pick_up_time(self, time: int):
        self._real_pick_up_time = time

    @property
    def real_wait_time(self) -> int:
        return self._real_wait_time

    @real_wait_time.setter
    def real_wait_time(self, time: int):
        self._real_wait_time = time
...
def generate_real_road_orders_data(output_file, *args, **kwargs):
    ...
    from setting import DAY
    shortest_distance = np.load("../data/Manhattan/network_data/shortest_distance.npy")
    order_data = pd.read_csv("../preprocess/raw_data/temp/Manhattan/order_data_{0:03d}.csv".format(DAY))
    ...
    order_data = order_data[["request_time", "wait_time", "pick_index", "drop_index", "order_distance", "order_fare", "n_riders"]]
    order_data.to_csv(output_file, index=False)

def generate_avg_real_road_orders_data(output_file, *args, **kwargs):
    """
    从工作日中随机选取平均数量订单
    :return:
    """
...
def generate_real_road_orders_data_for_learning(output_file, *args, **kwargs):
    """
    产生训练订单数据
    ...
    day = Day()
    print("generate day: ", day.day)
    shortest_distance = np.load("../data/Manhattan/network_data/shortest_distance.npy")
    order_data = pd.read_csv("../preprocess/raw_data/temp/Manhattan/week_day.csv")
    order_data = order_data[order_data.day==day.day]
    order_data = order_data[
        (MIN_REQUEST_TIME <= order_data.request_time) & (order_data.request_time < MAX_REQUEST_TIME)]
    # order_data = order_data[shortest_distance[order_data.pick_index, order_data.drop_index] != np.inf]
    # order_data = order_data[shortest_distance[order_data.pick_index, order_data.drop_index] >= 1000.0]  # 过于短的或者订单的距离是无穷大
    order_data["wait_time"] = np.random.choice(WAIT_TIMES, size=order_data.shape[0])
    # order_data["order_distance"] = shortest_distance[order_data.pick_index, order_data.drop_index]
...
#                 temp_order_data = temp_order_data[["request_time", "wait_time", "pick_index", "drop_index", "order_distance", "order_fare", "n_riders"]]
...
def generate_road_orders_data(output_file: str, network: Network):
    ...
    调用上面两类函数
    """
    generate_avg_real_road_orders_data(output_file, network)
