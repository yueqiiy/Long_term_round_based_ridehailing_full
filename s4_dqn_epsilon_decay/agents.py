...
from s4_dqn_epsilon_decay.modules import MLP
...
class DQNAgent(object):
    ...
    def __init__(self,q_func, optimizer, explorer,replay_buffer, batch_size, replay_start_size,update_target_steps, n_act, gamma=0.9):
        ...
        self.net = torch.load(PKL_PATH+".pkl")
        print("self.net path = ",PKL_PATH)
    ...
    def predict(self, obs):
        ...
        action = int(torch.argmax(Q_list).detach().numpy())  # 能实现
        return action

    def load_predict(self, obs):
        obs = torch.FloatTensor(obs)
        Q_list = self.net(obs)
        action = int(torch.argmax(Q_list).detach().numpy())  # 能实现
        return action
    ...
    def learn_batch(self, batch_obs, batch_action, batch_reward, batch_next_obs, batch_done):
        # predict_Q
        pred_Vs = self.pred_func(batch_obs)   # 可以输一组值进去吗？
