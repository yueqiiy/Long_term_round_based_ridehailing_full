...
# Author: Jackye
# Time : 2020/12/24 下午2:10

"""
用来导出起始点和终止点为同一个区域的订单
"""
...
result_dir = "raw_data/temp/Manhattan/same_zone"
    ...
    for csv_iterator in pd.read_table("./{0}_tripdata_2019-06.csv".format(color), chunksize=chunk_size, iterator=True):
        ...
        for line in csv_iterator.values:
            ...
            if color == green:
                ...
                s[6] = order_time
                s = s[:7]
                # 剔除不在曼哈顿地区的订单；剔除起始点终点不再同一个区域的订单；剔除行程距离为0的订单
                # 保留了行驶时间很短的订单
                if int(s[4]) != int(s[5]) or \
                        int(s[4]) not in manhattan_zone_id_set or \
                        int(s[5]) not in manhattan_zone_id_set or float(s[2]) == 0:
                    continue
